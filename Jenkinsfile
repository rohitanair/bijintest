...
node 
{
   ...
   stage('Checkout')
   {
       ...
       sh 'oc set image --local=true -f Orchestration/deployment.yaml ${MS_NAME}=${DOCKER_REGISTRY}/sprint14project-dev-apps/${MS_NAME}:sprint14project-dev-apps --dry-run -o yaml >> Orchestration/deployment-sprint14project-dev.yaml'
  ...
  if(env.SECURITY_TESTING == 'True')
  {
   ...
   stage('Build and Tag Image for Dev')
   {
	   ...
	   FAILED_STAGE=env.STAGE_NAME
	   sh 'oc config use-context sprint14project-devops'
	  node('docker')
    {
        ...
        container('jnlp')
        {
            ...
            sh "docker build -t ${MS_NAME}:latest ."
             sh 'docker tag ${MS_NAME}:latest ${DOCKER_REGISTRY}/${DOCKER_REPO}/$MS_NAME:sprint14project-dev-apps'
           sh 'docker login ${DOCKER_REGISTRY} --username $username  --password $password'
			sh 'docker push ${DOCKER_REGISTRY}/${DOCKER_REPO}/$MS_NAME:sprint14project-dev-apps'
   ...
   {   
       ...
       /*sh 'oc config use-context sprint14project-dev-apps'
       sh 'oc config view'
       sh 'oc apply -f Orchestration/deployment-sprint14project-dev.yaml -n=sprint14project-dev-apps'
       sh 'oc apply -f Orchestration/service.yaml -n=sprint14project-dev-apps' */
